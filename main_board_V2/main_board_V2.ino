...
//CAPTEUR INFRAROUGE// 
const int capteur_infrarouge_a = 3;
const int capteur_infrarouge_b = 2;
...
void setup() {
  ...
  myNex.begin(9600);  // jsp trop pour initialisé le baud rate de l'écrans d'après gpt
  Serial3.begin(9600); // For communication with Arduino Mega 2
...
void loop() {
  ...
  if (Serial3.available() > 0) {
    String receivedSpeed = Serial3.readStringUntil('\n');
    Serial.print("receivedSpeed: ");
    Serial.print(receivedSpeed);
        Serial.print("  receivedSpeed.length(): ");
    Serial.print(receivedSpeed.length());
    //if (receivedSpeed.length() == 4) {
      char identifier = receivedSpeed.charAt(0);
      int speedValue = receivedSpeed.substring(1).toInt();
  ...
  if (tempsActuel - derniereActivation > 300) { 
    ...
    if (digitalRead(boutonReset_a) == LOW) {
      ...
      while ((tempsActuel - derniereActivation) < 5000 ){
        ...
        if ((digitalRead(boutonReset_a) == LOW) && (temps < 5) ) {
          bool reset = true ;
      ...
      while ((tempsActuel - derniereActivation) < 5000 ){
        ...
        int temps = (5-((tempsActuel - derniereActivation) / 1000));
        myNex.writeNum("n6.val", temps );
        if ((digitalRead(boutonReset_b) == LOW) && (temps < 5) ) {
          bool reset = true ;
          myNex.writeStr("page page0");
          myNex.writeStr("page page0"); 
          delay(200);
